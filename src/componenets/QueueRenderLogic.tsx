...
interface IQueueRenderLogicProps {
}

const QueueRenderLogic: React.FunctionComponent<IQueueRenderLogicProps> = () => {
  const { queue } = React.useContext(QueueContext)
  const hash = React.useRef(new Map())

   //QueueS EVENTS AND DEPENDENCIES
...
const handleDragEnd = (e: any) => {
  ...
  return (
    <React.Fragment>
      {(queue.value.length > 0) && queue.value.map((obj) => {
        return (
...
export default QueueRenderLogic;
